...
public class StreamInAction {
    ...
    public static void main(String[] args) {
        ...
        //1.Find all transactions the year 2011 and sort them by value (small to high)
        transactions.stream()
...
//                .findAny().filter(t->t.getCity().equals("Milan"))
...
//                .collect(toList())
        ...
        Integer maxInt = transactions.stream()
                .map(Transaction::getValue).reduce(Integer::max).get();
//                .reduce((i, j) -> {
//                    return i > j ? i : j;
//                }).get();
        ...
        Integer smallInt = transactions.stream()
                .map(Transaction::getValue)
